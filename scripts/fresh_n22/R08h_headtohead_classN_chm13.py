...
def class_guides_in_windows(cls):
    ...
    cg=set()
    for f in glob.glob(f"{H}/ind_targets/{cls}/*.fasta")+glob.glob(f"{H}/ind_targets/{cls}/*.fa"):
...
def crispor(cls):
    d={}
    for f in glob.glob(f"{H}/ind_crispor_class_scores/{cls}/*_crispor.tsv"):
        with open(f) as fh:
            for ln in fh:
...
def guidescan(cls):
    d={}
    # R07h writes {cls}.csv; older runs used {cls}_ont.csv -- accept either
    base=f"{H}/ind_guidescan_class_scores/out"
...
for cls in CLASSES:
    ...
    print()
pd.DataFrame(rows).to_csv(f"{H}/headtohead_classN_human_IND.tsv",sep="\t",index=False)
print(f"wrote {H}/headtohead_classN_human_IND.tsv")
