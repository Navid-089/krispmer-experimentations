...
# Writes /mnt/windows-data/Thesis/plots/fixed/fig_h2h_combined.png
...
#
# SOURCE OF TRUTH = the two VM TSVs (headtohead_classN_human_bothstrand.tsv /
# headtohead_classN_n22_bothstrand.tsv). Read if found; else the baked-in fallbacks below,
...
# BOTH-STRAND fallbacks (must match data/headtohead_classN_*_bothstrand.tsv)
_HUMAN_FB={
...
_RICE_FB={
"multiple_v_single":    {"N":133,"kRISP-meR":(7,121,5),"CRISPOR":(64,69,0),"GuideScan2":(107,26,0)},
"ref_present_v_absent": {"N":73, "kRISP-meR":(2,10,61),"CRISPOR":(35,38,0),"GuideScan2":(56,17,0)},
"ref_multiple_v_single":{"N":109,"kRISP-meR":(38,70,1),"CRISPOR":(10,99,0),"GuideScan2":(85,24,0)},
"present_v_absent":     {"N":237,"kRISP-meR":(161,67,9),"CRISPOR":(140,97,0),"GuideScan2":(0,0,237)},
}
HUMAN=_load([os.path.expanduser("~/human_assemblies_kmer_count/july-10/headtohead_classN_human_bothstrand.tsv"),
             os.path.join(os.path.dirname(__file__),"headtohead_classN_human_bothstrand.tsv"),
             "/mnt/windows-data/Thesis/data/headtohead_classN_human_bothstrand.tsv"], _HUMAN_FB)
RICE=_load([os.path.expanduser("~/krispmer/new_datasets/exps/n22/fresh/headtohead_classN_n22_bothstrand.tsv"),
            os.path.join(os.path.dirname(__file__),"headtohead_classN_n22_bothstrand.tsv"),
            "/mnt/windows-data/Thesis/data/headtohead_classN_n22_bothstrand.tsv"], _RICE_FB)
...
fig.savefig(f"{OUT}/fixed/fig_h2h_combined.png",dpi=200,bbox_inches="tight"); plt.close()
print(f"wrote {OUT}/fixed/fig_h2h_combined.png")
