...
for CLS in $CLASSES; do
  mkdir -p "scores_bothstrand/$CLS" "logs_bothstrand/$CLS"
  for t in targets/$CLS/target_*.fasta targets/$CLS/target_*.fa; do
    [ -e "$t" ] || continue
    name=$(basename "$t" | sed 's/\.\(fasta\|fa\)$//')
    out="scores_bothstrand/$CLS/${name}.csv"
    [ -s "$out" ] && { echo "[skip] $CLS/$name"; continue; }
    echo "[run ] $CLS/$name  -ein -n"
    krispmer "${READS:-$t}" "$t" "$out" "$MAX_HD" \
      -J "$JF" -H "$HISTO" -ein "$EM" -n \
      -l "logs_bothstrand/$CLS/${name}.log" || echo "[FAIL] $CLS/$name"
  done
done
