...
def krisp(cls):
    d={}
    for f in glob.glob(f"{H}/scores_bothstrand/{cls}/*.csv"):
...
for cls in CLASSES:
    ...
    print()
pd.DataFrame(rows).to_csv(f"{H}/headtohead_classN_human_bothstrand.tsv",sep="\t",index=False)
print(f"wrote {H}/headtohead_classN_human_bothstrand.tsv")
