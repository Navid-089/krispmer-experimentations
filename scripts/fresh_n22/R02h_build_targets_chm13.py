...
FRESH   = os.path.expanduser("~/human_assemblies_kmer_count/july-10")
BUCKET  = os.path.expanduser(f"~/human_assemblies_kmer_count/july-10/{CLASS}.txt")
EXONS   = f"{FRESH}/chm13_exons.fa"
IDX     = f"{FRESH}/chm13_exons_idx"
OUTDIR  = f"{FRESH}/ind_targets/{CLASS}"
SAMOUT  = f"{FRESH}/ind_{CLASS}_exons_max_0.sam"
GENES   = f"{FRESH}/ind_genes_{CLASS}_analysis.txt"
os.makedirs(OUTDIR, exist_ok=True)

def rc(s): return s.translate(str.maketrans("ACGT","TGCA"))[::-1]

# ---- 1. build a bowtie query FASTA of the class guides (both strands) -------
qry = f"{FRESH}/ind_{CLASS}_guides.fa"
ng = 0
...
print(f"[{CLASS}] bowtie -v 0 vs n22_exons ...")
...
with open(SAMOUT) as fh:
    for ln in fh:
        ...
        # CHM13 CAT/GENCODE tx id = ENST00000166534.8 -> gene = strip .version
        gene = tx.rsplit(".",1)[0]
        genes.add(gene)
