...
# design both strands). Writes headtohead_classN_n22_bothstrand.tsv so the
...
def krisp(cls):
    d={}
    for f in glob.glob(f"{FRESH}/scores_bothstrand/{cls}/*.csv"):
...
for cls in CLASSES:
    ...
    print()
pd.DataFrame(rows).to_csv(f"{FRESH}/headtohead_classN_n22_bothstrand.tsv",sep="\t",index=False)
print(f"wrote {FRESH}/headtohead_classN_n22_bothstrand.tsv")
