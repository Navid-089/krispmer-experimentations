...
def load(tsv):
    ...
    return C
IND =load("/mnt/windows-data/Thesis/data/headtohead_classN_human_IND.tsv")  # N22-anchored
REF =load("/mnt/windows-data/Thesis/data/headtohead_classN_human_bothstrand.tsv")         # japonica-anchored
...
fig.text(0.008,0.72,"Targets from\nCHM13 (individual)",ha="left",va="center",fontsize=10.5,color="#2b2f3a",weight="bold")
fig.text(0.008,0.30,"Targets from\nGRCh38 (reference)",ha="left",va="center",fontsize=10.5,color="#2b2f3a",weight="bold")
fig.suptitle("Human: target windows anchored to the individual (CHM13) vs the reference (GRCh38)",
...
fig.savefig(f"{OUT}/fig_anchor_comparison_human.png",dpi=200,bbox_inches="tight"); plt.close()
print(f"wrote {OUT}/fig_anchor_comparison_human.png")
