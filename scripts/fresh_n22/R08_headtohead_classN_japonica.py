#!/usr/bin/env python3
# 08b_headtohead_classN_n22_bothstrand.py — same as 08_headtohead_classN_n22.py but
# kRISP-meR is read from scores_bothstrand/ (the run WITH -n/--consider_negative,
# i.e. both strands designed). CRISPOR/GuideScan paths unchanged (they already
# design both strands). Writes headtohead_classN_n22_REF.tsv so the
# +strand-only vs both-strand "not-generated" fractions can be compared directly.
#
...
def class_guides_in_windows(cls):
    ...
    cg=set()
    for f in glob.glob(f"{FRESH}/ref_targets/{cls}/*.fasta"):
...
def krisp(cls):
    d={}
    for f in glob.glob(f"{FRESH}/ref_scores_bothstrand/{cls}/*.csv"):
...
def crispor(cls):
    d={}
    for f in glob.glob(f"{FRESH}/ref_crispor_class_scores/{cls}/*_crispor.tsv"):
        with open(f) as fh:
            for ln in fh:
...
def guidescan(cls):
    d={}
    fn=f"{FRESH}/ref_guidescan_class_scores/out/{cls}_ont.csv"
...
for cls in CLASSES:
    ...
    print()
pd.DataFrame(rows).to_csv(f"{FRESH}/headtohead_classN_n22_REF.tsv",sep="\t",index=False)
print(f"wrote {FRESH}/headtohead_classN_n22_REF.tsv")
